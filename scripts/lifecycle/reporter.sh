...
# 2. 读取当前目标
echo "📋 当前目标"
if [ -f "$AI_MEMORY_DIR/STATUS.md" ]; then
    grep -A 2 "当前目标" "$AI_MEMORY_DIR/STATUS.md" | head -3 | sed 's/^- //'
else
    echo "状态文件不存在"
fi
echo ""
...
if [ -f "$AI_MEMORY_DIR/TASK_QUEUE.md" ]; then
    grep -E "^- \[ \]|^- \[x\]" "$AI_MEMORY_DIR/TASK_QUEUE.md" | head -3 | sed 's/^- //'
else
    echo "任务队列文件不存在"
fi
echo ""
...
echo "🚧 阻塞项"
if [ -f "$AI_MEMORY_DIR/STATUS.md" ]; then
    grep -A 2 "阻塞项" "$AI_MEMORY_DIR/STATUS.md" | head -3 | sed 's/^- //'
else
    echo "无阻塞项信息"
fi
echo ""
...
if [ -f "$AI_MEMORY_DIR/${TODAY}.md" ]; then
    tail -10 "$AI_MEMORY_DIR/${TODAY}.md"
else
    echo "今日暂无日志"
fi
echo ""
...
if [ ! -f "$AI_MEMORY_DIR/TASK_QUEUE.md" ]; then
    echo "请检查任务队列文件"
elif grep -q "^\- \[ \]" "$AI_MEMORY_DIR/TASK_QUEUE.md"; then
    echo "有待办任务需要处理"
    grep "^\- \[ \]" "$AI_MEMORY_DIR/TASK_QUEUE.md" | head -1 | sed 's/^- //'
else
